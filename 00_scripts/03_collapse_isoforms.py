...
def filter_out_subsets(recs, internal_fuzzy_max_dist):
    # recs must be sorted by start because that's the order they are written
    i = 0
    while i < len(recs) - 1:
        no_change = True
        j = i + 1
...
def collapse_isoforms(gff_filename, fasta_filename, output_folder, name):
    ...
    good = []
    output_gtf_filename = os.path.join(output_folder, f'{name}_corrected.5degfilter.gff')
    ...
    SeqIO.write(isoform_seqs, os.path.join(output_folder, f'{name}_corrected.5degfilter.fasta'), 'fasta')
...
def main():
    ...
    collapse_isoforms(args.sqanti_gtf, args.sqanti_fasta, args.output_folder, args.name)
